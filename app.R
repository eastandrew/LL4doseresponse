...
ui <- fluidPage(
    ...
    # App title ----
    titlePanel("SWLL4, StageWise 4-param Logistic (\"Hill\") Dose Response Model and Dose Selection"),
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            ...
            textInput("dose", label = h3("Dose Input"), value = "0,100,350,900"),
            textInput("response", label=h3("Response Input"), value="0,1,2,5"),
            textInput("total", label=h3("Total Animals Input"), value="5,5,5,5"),
            ...
            br(),
            p("Written by A. East"),
            p(a("GitHub",href="https://github.com/eastandrew/LL4doseresponse"))
...
server <- function(input, output) {
    ...
    d <- eventReactive(input$run, {
        ...
        drm(response/total~dose, data=df,fct=LL.4(fixed=c(NA,0,1,NA),names=c("Slope","Min","Max","LC50")), type="binomial", weights=total)
